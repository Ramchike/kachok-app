...
import { addExerciseToListAtom, Difficulty, difficultyOptions, equipmentList, exerciseListAtom, tagList } from '../../../storage/exercise';
...
export const AddExerciseModal: React.FC<Props> = ({ isOpen, onClose}) => {
  const [, addExercise] = useAtom(addExerciseToListAtom)
  ...
  const addNewExercise = (imageLink?: string) => {
    addExercise({
      id: exerciseList.length > 0 ? exerciseList[exerciseList.length - 1].id + 1 : 0,
      difficulty,
  ...
  const handleSubmit = () => {
    if (imageFile) {
      uploadImage(imageFile)
        .then(imageLink => addNewExercise(imageLink))
        .catch(() => addNewExercise());
    } else {
      addNewExercise();
    }
  };
      ...
      >
        ...
        >
          <div className="flex-shrink-0 flex justify-between items-center p-4 border-b border-gray-200">
            <h2 className="text-lg dark:text-black font-bold">Новое упражнение</h2>
            <button
              onClick={onClose}
            ...
            <div className="p-4 space-y-6">
              ...
              >
                ...
                <input
                  ...
                  className="w-full dark:text-black px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  required
                />
              </motion.div>
              ...
              >
                ...
                <textarea
                  ...
                  className="w-full resize-none dark:text-black px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  rows={4}
                />
              </motion.div>
              ...
              >
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-600 mb-2">Сложность</label>
          ...
          <div className="flex-shrink-0 flex gap-3 p-4 border-t border-gray-200">
            <motion.button
              ...
              type="button"
              onClick={onClose}
              className="flex-1 py-3 dark:text-black border border-gray-300 rounded-xl hover:bg-gray-50 active:bg-gray-100 touch-manipulation font-medium transition-colors duration-200"
            >
              Отмена
            </motion.button>
            <motion.button
